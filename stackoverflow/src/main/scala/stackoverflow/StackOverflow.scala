...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    ...
    val answers: RDD[(QID, Answer)] = postings.filter(_.postingType == 2)
        .flatMap(post => post.parentId match {
          case Some(id) => Seq((id, post))
          case _ => Seq()
        })
    val joined = questions.join(answers)
    val grouped = joined.groupByKey().persist()
    grouped
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    val result = grouped.map { case (qid, iter) =>
      val qa = iter.toArray
      val answers = qa.map(_._2)
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] =
    ...
    val result: RDD[(LangIndex, HighScore)] = scored.flatMap{ case (question, score) =>
      firstLangInTag(question.tags, langs) match {
        case Some(id) => Seq((id * langSpread, score))
        case _ => Seq()
      }}
    result
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] =
      val groupedVectors = vectors.map(p => (findClosest(p, means), p)).groupByKey()
      groupedVectors.mapValues(v => averageVectors(v)).collect().toMap
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val vsList = vs.toList.sortBy(_._2)
    val count = vsList.size
    val sorted = vsList.groupBy(_._1).map { case (langIndex, list) => (langIndex, list.length) }.toList.sortBy(_._2).reverse
    val langLabel: String = langs(sorted.head._1/langSpread) // most common language in the cluster
    val langPercent: Double = sorted.head._2 / count * 100 // percent of the questions in the most common language
    val clusterSize: Int = count
    val medianScore: Int = {
     if (count % 2 == 0) {
       val l = count / 2 - 1
       val r = l + 1
       (vsList(l)._2 + vsList(r)._2) / 2
     } else {
       vsList(count / 2)._2
     }
    }
